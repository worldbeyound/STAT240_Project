...
What separates a super bowl winner from the rest? Are there any unique characteristics that are needed in order for a team to win the super bowl? In this analysis we seek to answer these questions by analyzing the super bowl winners from the last 10 years. Specifically we are looking at whether (blank variables) differ in super bowl winners compared to the rest of the NFL. 
 ...
 Question Interest: How does the average of blank variables of superbowl winners in the last ten years compare to the rest of the NFL? And
...
# Variables  
  We focused on just four variables out of several thousand others. The first Variable is air yards divided by yards after catch. This variable was constructed to represent the teams aggressiveness when they are backed up in their own 30 yard down. Air yards represents the distance the ball travels in the air to the target. Yards after catch represents the distance the player travels after receiving the catch. Air yards divided by yards after catch will represent the teams aggressiveness under pressure, this will be referred to more simply as the offensive aggressiveness stat.
  The next variable was constructed to represent defensive success-specifically in the defending red zone (less than 20 yards). By dividing tackles for a loss in the Red Zone by tackles for a loss not in the Red Zone we created a proportion.
  Variables three and four are simpler. Variable three simply represents third down conversion rate and variable four represents total touchdowns per game. Keep in mind for each variable there are values for its corresponding year and the Superbowl winner and the rest of the NFL.
  This leads to our central questions of interest. 
  How does offensive aggressiveness, tackles for a loss, third down conversion rate and touchdowns per game change differ between the Superbowl winner and the rest of the NFL? What conclusions, if any, can we make about future Superbowl winners?
  We seek to answer these questions using the One Sample t-test.
...
Clearly the graph indicates some difference between the NFL and the Superbowl winners. We have Superbowl winners who have a higher offensive aggressiveness, but also, a couple who are starkly lower than what seems to be the average around 1.4. To further analyze the data it's important to interpret the results from the t-tests. The mean of the Superbowl winners is slightly higher than the rest of the NFL. Furthermore, the confidence interval allows us to predict that the next super bowl winner will likely have an offensive aggressiveness stat between 1.37 and 1.82.
...
This graph proposes the idea that superbowl winners are more often differnt than the rest of the NFL, but there isn't a clear clustering of data around a certain value. The t-test supports this notion of their being no clear difference one way or another between Superbowl winners and the rest of the NFL. The means are quite close, and the p-value is closer to .05 than we would expect, and their seems to be quite a bit of variance as reflected in the confidence interval. The next superbowl winner could have a proportion for tackles for a loss in the Red Zone from .69 to 1.69. 
...
The graphs clearly show higher third down conversion rates than the rest of the NFL. The p-value from the t-test supports this conclusion because it is lower than .05. Furthermore, we can infer that the true mean third down conversion rate of past and future Superbowl winners is between .39 and .47. Whereas the rest of the NFL would average somewhere between .38 and .40. 
...
Similar to third down conversion rate, visually, there is a difference in touchdowns per game for the Superbowl winner and the rest of the NFL. Additionally, the t-test suggests that the next superbowl winner has about 2-3 touchdowns per game.

### Discussion-Answer Questions of Interest
Three out of the four variables show apparent differences in the Superbowl winner and the rest of the NFL, Third down conversion rate, touchdowns per game, and offensive aggressiveness. Supported by t-tests, we were accurately able to infer a range of values for each variable in which the next Superbowl winner will take on. 

#Future Research
The present study could be used as a possible starting point for predicting the next Superbowl winner. One could conduct these kinds of tests for the thousands of variables included in the data sets. Then, at the end of the next season they could search for team(s) that have values included in the confidence intervals to make an accurate guess on who the next winner will be. 
